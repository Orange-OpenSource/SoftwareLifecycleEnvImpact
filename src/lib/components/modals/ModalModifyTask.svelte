...
<button
	...
	data-bs-target="#modalModifyTask{task.id}"
	type="button"
	class="btn btn-outline-primary btn-sm {classAttribute}">Modify</button
>
		...
		<select class="form-select" aria-label="Default select example">
			{#each tasks as currentTask}
				{#if currentTask.id !== task.id}
					<option on:click={() => updateParent(currentTask.id)} selected={isParent(currentTask)}
						>{currentTask.name}</option
					>
				{/if}
			{/each}
		</select>
