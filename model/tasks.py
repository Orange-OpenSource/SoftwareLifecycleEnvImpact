...
class Task(ABC):
    def __init__(self, resources: List[Resource] = None, subtasks: List[Task] = None):
...
class RunTask(Task):
    def __init__(
        self, maintenance_days: int, user_hours: int, server_hours: int, storage_hours: int, network_gb: int
    ) -> None:
