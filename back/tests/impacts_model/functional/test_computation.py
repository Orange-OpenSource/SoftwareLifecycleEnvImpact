...
def test_build_task_impacts() -> None:
    ...
    # Dev days
    old_co2 = s.build_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE)
    s.dev_days = 4432
    assert (
        s.build_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE) != old_co2
    )
...
def test_run_task_impacts() -> None:
    ...
    # Maintenance days
    old_co2 = s.run_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE)
    s.maintenance_days = 4432
    assert s.run_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE) != old_co2
    ...
    s.avg_user = 4432
    assert (
        s.root_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE) != old_co2
    )
...
def test_dev_task_impacts() -> None:
    ...
    s = StandardProject()
    old_co2 = s.dev_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE)
    s.dev_days = 234832443
    assert s.dev_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE) != old_co2


def test_design_task_impacts() -> None:
    """Test co2 consequence of updating all project's inputs for design task"""
    s = StandardProject()
    old_co2 = s.design_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE)
    s.design_days = 23123
    assert (
        s.design_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE) != old_co2
    )
...
def test_spec_task_impacts() -> None:
    ...
    s = StandardProject()
    old_co2 = s.spec_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE)
    s.spec_days = 214421
    assert (
        s.spec_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE) != old_co2
    )
...
def test_implementation_task_impacts() -> None:
    ...
    # Dev days
    old_co2 = s.implementation_task.get_impact_by_category(
        ImpactCategory.CLIMATE_CHANGE
    )
    s.dev_days = 1244
    assert (
        s.implementation_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE)
        != old_co2
    )
...
def test_management_task_impacts() -> None:
    ...
    s = StandardProject()
    old_co2 = s.management_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE)
    s.management_days = 3243
    assert (
        s.management_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE)
        != old_co2
    )
...
def test_maintenance_task_impacts() -> None:
    ...
    s = StandardProject()
    old_co2 = s.maintenance_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE)
    s.maintenance_days = 234234
    assert (
        s.maintenance_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE)
        != old_co2
    )
...
def test_hosting_task_impacts() -> None:
    ...
    # Electricity mix
    old_co2 = s.hosting_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE)
    ImpactsSourceRegistry().electricity_mix = 2.12312312 * ELECTRICITY_MIX
    assert (
        s.hosting_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE)
        != old_co2
    )
...
def test_usage_task_impacts() -> None:
    ...
    # Avg user
    old_co2 = s.usage_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE)
    s.avg_user = 5325235435
    assert (
        s.usage_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE) != old_co2
    )

    # Avg time   old_co2 = s.root_task.run_task.usage_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE)
    s.avg_time = 64
    assert (
        s.usage_task.get_impact_by_category(ImpactCategory.CLIMATE_CHANGE) != old_co2
    )
